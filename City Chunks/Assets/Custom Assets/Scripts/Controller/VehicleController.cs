...
public class VehicleController : MonoBehaviour {
  ...
  public void Awake() { GameData.numVehicles++; }
  public void Start() {
    rbody = GetComponent<Rigidbody>();
    fuelRemaining = startFuel;
  }

  public void UpdateInputs(float forward, float turn, float lookH, float lookV,
  ...
  public void OnDestroy() {
    if (GameData.numVehicles < 5) {
      PlayerController[] players = FindObjectsOfType<PlayerController>();
      if (players.Length > 0) {
        GameObject player = players[Random.Range(0, players.Length)].gameObject;
