...
public class NPCController : MonoBehaviour {
  ...
  [HideInInspector] public Vector3 targetDashDirection;
  ...
  void Start() {
    ...
    agent.enabled = false;
    meshBuilder = GetComponent<LocalNavMeshBuilder>();
    spawnTime = Time.time;
  }
  ...
  void Update() {
    ...
    if (animator) {
      ...
      //   }
      // }
      if (Input.GetButtonDown("FireL") && canAction && isGrounded) {
        Attack(1);
      }
      if (Input.GetButtonDown("FireR") && canAction && isGrounded) {
        Attack(2);
      }
      if (Input.GetButtonDown("FireL") && Input.GetAxisRaw("Sprint") != 0 &&
          canAction && isGrounded && !isStrafing) {
        AttackKick(1);
      }
      if (Input.GetButtonDown("FireR") && Input.GetAxisRaw("Sprint") != 0 &&
          canAction && isGrounded && !isStrafing) {
        AttackKick(2);
      }
      // if strafing
      // if (Input.GetKey(KeyCode.LeftAlt) ||
      //     Input.GetAxisRaw("TargetBlock") > .1 && canAction) {
      //   isStrafing = true;
      ...
      if (agent.isOnNavMesh) {
        ...
        if (meshBuilder != null &&
            Vector3.Distance(agent.destination, goal) >
                meshBuilder.m_Size.x / 2.1f) {
          agent.destination = transform.TransformPoint(
              Vector3.ClampMagnitude(transform.InverseTransformPoint(goal),
                                     meshBuilder.m_Size.x / 2.1f));
        } else {
  ...
  // get velocity of rigid body and pass the value to the animator to control
  // the animations
  void LateUpdate() {
    if (!useNavMesh) {
      ...
      float agentVelX = transform.InverseTransformDirection(agent.velocity).x;
      float agentVelZ = transform.InverseTransformDirection(agent.velocity).z;
      float velX = agentVelX / walkSpeed / 2f;
      if (agentVelX > walkSpeed) {
        velX = 0.5f + (agentVelX - walkSpeed) / (runSpeed - walkSpeed) / 2f;
      }
      float velZ = agentVelZ / walkSpeed / 2f;
      if (agentVelZ > walkSpeed) {
        velZ = 0.5f + (agentVelZ - walkSpeed) / (runSpeed - walkSpeed) / 1f;
      }
  ...
  void CameraRelativeMovement() {
    ...
    float inputDashHorizontal = Input.GetAxisRaw("Dash") * inputHorizontal;
    float inputDashVertical = Input.GetAxisRaw("Dash") * inputVertical;
    ...
    // Right vector relative to the camera always orthogonal to the forward
    // vector
  ...
  // checks if character is within a certain distance from the ground, and
  // markes it IsGrounded
  void CheckForGrounded() {
    if (!useNavMesh) {
      ...
      if (Physics.Raycast((transform.position + offset), -Vector3.up, out hit,
                          100f)) {
  ...
  void Jumping() {
    if (isGrounded) {
      ...
      if (canDoubleJump && doublejumping && Input.GetButtonDown("Jump") &&
          !doublejumped && isFalling) {
  ...
  void AirControl() {
    if (!isGrounded) {
      ...
      Vector3 motion = inputVec;
      motion *=
          (Mathf.Abs(inputVec.x) == 1 && Mathf.Abs(inputVec.z) == 1) ? 0.7f : 1;
        ...
        if (isGrounded) {
          if (attackSide != 3) {
            animator.SetTrigger("Attack" + (attackNumber).ToString() +
                                "Trigger");
            if (leftWeapon == 12 || leftWeapon == 14 || rightWeapon == 13 ||
                rightWeapon == 15) {
            ...
            animator.SetTrigger("AttackDual" + (attackNumber).ToString() +
                                "Trigger");
            StartCoroutine(_LockMovementAndAttack(0, .75f));
          }
        }
  ...
  public void CastAttack(int attackSide) {
    if (weapon == Weapon.UNARMED) {
      ...
      if (attackSide == 1) {
        ...
        if (isGrounded) {
          animator.SetTrigger("CastAttack" + (attackNumber + 1).ToString() +
                              "Trigger");
          StartCoroutine(_LockMovementAndAttack(0, .8f));
        }
      }
      ...
      if (attackSide == 3) {
        int attackNumber = Random.Range(0, maxAttacks);
        if (isGrounded) {
          animator.SetTrigger("CastDualAttack" + (attackNumber + 1).ToString() +
                              "Trigger");
  ...
  IEnumerator _Knockback(Vector3 knockDirection, int knockBackAmount,
                         int variableAmount) {
    isKnockback = true;
    StartCoroutine(
        _KnockbackForce(knockDirection, knockBackAmount, variableAmount));
    yield return new WaitForSeconds(.1f);
    isKnockback = false;
  }

  IEnumerator _KnockbackForce(Vector3 knockDirection, int knockBackAmount,
                              int variableAmount) {
    while (isKnockback) {
      rb.AddForce(
          knockDirection * ((knockBackAmount +
                             Random.Range(-variableAmount, variableAmount)) *
  ...
  public IEnumerator _DirectionalRoll(float x, float v) {
    ...
    float sign = Mathf.Sign(Vector3.Dot(
        transform.up, Vector3.Cross(targetDashDirection, transform.forward)));
    // angle in [-179,180]
